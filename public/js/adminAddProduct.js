document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!cover) {
        showError("coverError", "Front cover required");
        valid = false;
    }

    if (!side) {
        showError("sideError", "Side image required");
        valid = false;
    }

    if (!back) {
        showError("backError", "Back image required");
        valid = false;
    }
    ...
    // Images (already processed via canvas)
    formData.append("coverImage", cover);
    formData.append("sideImage", side);
